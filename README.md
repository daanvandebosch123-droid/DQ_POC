...
The **Anomalies tab** goes further: pick a connection and a file/table, run a check, and it profiles every source column (row count, SQL-null rate, blank/space-only rate for text, distinct count, inferred meaning, min-max, mean, text length, etc.) and compares it against the previous snapshot for drift — row count jumps, null spikes, vanished columns, shifted averages, malformed values, and outliers. Text min/max use alphabetical order, while detected dates use chronological order. **Distinct %** is calculated against non-null values; it is not a claim that every value occurs exactly once. Each run adds a snapshot, so null-rate and row-count trend charts build up over time.
