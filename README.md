...
AI features (anomaly explanations, rule drafting, recommendations) are optional and use an [Ollama](https://ollama.com) server. By default the app connects to the team's shared Ollama endpoint at `https://ollama.dqpocai.org` (default model `qwen3:8b`) through Cloudflare Access, authenticated with a service token — set the `CF-Access-Client-Id` / `CF-Access-Client-Secret` pair via **AI settings** in the sidebar (stored in the OS credential store, never in a file or Git). Endpoint and model are also configurable there, including pointing at a plain local Ollama instead (e.g. `http://localhost:11434`). None of the AI buttons do anything useful without a reachable endpoint; everything else in the app works fine either way. Because the default endpoint is remote, requests do leave this machine — only the "include sample values" option in Draft-with-AI sends actual source values, and it is off by default.
...
The **Anomalies tab** goes further: pick a connection and a file/table, run a check, and it profiles the source (row count, SQL-null rate, blank/space-only rate for text, distinct count, inferred meaning, min-max, mean, text length, etc.) and compares it against the previous snapshot for drift — row count jumps, null spikes, vanished columns, shifted averages, malformed values, and outliers. Text min/max use alphabetical order, while detected dates use chronological order. **Distinct %** is calculated against non-null values; it is not a claim that every value occurs exactly once. For predictable query size, profiling is limited to the first 50 columns and the app shows a notice when more columns exist. Each run adds a snapshot, so null-rate and row-count trend charts build up over time.
