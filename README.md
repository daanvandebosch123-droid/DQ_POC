...
AI features (anomaly explanations, rule drafting, recommendations) are optional and use an [Ollama](https://ollama.com) server. By default the app connects to a local Ollama endpoint at `http://localhost:11434` using model `qwen3:8b`, so install Ollama and run `ollama pull qwen3:8b` before using the AI buttons. The endpoint and model are configurable through **AI settings** in the sidebar. Remote endpoints protected by Cloudflare Access are also supported by saving a `CF-Access-Client-Id` / `CF-Access-Client-Secret` pair there (stored in the OS credential store, never in a file or Git). Everything except the optional AI features works without a reachable Ollama endpoint. Requests stay on this machine when the local endpoint is used.
...
The unit-test map and conventions for adding tests are in the
[technical documentation](docs/technical-documentation.md#testing-and-quality-checks).
...
- **Data Volume (Row Count)** — checks the source's row count against a min/max range
...
- **Data Freshness** — fails when the newest date/timestamp value is older than the allowed age, or the source has no readable latest date
...
The **Anomalies tab** goes further: pick a connection and one or more files/tables, run checks, and it profiles every source column (row count, SQL-null rate, blank/space-only rate for text, distinct count, inferred meaning, min-max, mean, text length, etc.) and compares it against the previous snapshot for drift — row count jumps, null spikes, vanished columns, shifted averages, malformed values, and outliers. Up to three selected sources are profiled concurrently; the batch table reports queued, running, completed, failed, or aborted status independently for each source. A per-source progress bar shows the current profiling stage, percentage complete, and percentage remaining. **Abort** prevents queued sources from starting and asks active profiles to stop at their next safe boundary; an in-flight database operation may take a moment to return, and aborted profiles never create a snapshot. Click a completed source to inspect its report, history, exports, and advisory AI actions. Database row counts, null/blank rates, distinct counts, and native-type aggregates remain full-source calculations. To keep text-type inference responsive, the default scan is capped at 50,000 rows and uses incremental counters instead of retaining values in memory. The profile shows the scanned/value counts, match confidence, and labels sample-derived inferred ranges. Select **Deep text scan** before a run only when full-table text inference is required. CSV profiling retains its existing full-source behaviour. Text min/max use alphabetical order, while detected dates use chronological order. **Distinct %** is calculated against non-null values; it is not a claim that every value occurs exactly once. Each successful run adds a snapshot, so null-rate and row-count trend charts build up over time.

The profiler proposes editable starter rules for complete fields, unique or duplicated identifier-like fields, date validity, email format, observed numeric ranges, text lengths, and small category lists. Suggestions can be filtered by field, then opened in the normal rule editor; they are never saved automatically. Identifier detection recognises common Dutch names such as `klantnummer`, `artikelnummer`, `ordernummer`, and suffixes including `nummer`, `nr`, `code`, and `sleutel`. **Explain with AI** summarizes the findings, while **Get AI recommendations** prioritizes possible rule types and next steps. Both send metadata only—not source rows or example values—to a locally running Ollama model. AI recommendations are advisory: use the normal Rule Ideas flow to review and save a rule.
...
Database connection passwords (Oracle, SQL Server, DB2, and Sybase) are stored locally per Windows user in Windows Credential Manager, rather than in the shared project database. Existing plaintext entries from `%LOCALAPPDATA%\DQTool\secrets.json` are migrated the first time each connection is used, then removed from that file. Login passwords are separate: those are salted PBKDF2-SHA256 hashes in `dqtool_workspace.sqlite`, never plaintext.
