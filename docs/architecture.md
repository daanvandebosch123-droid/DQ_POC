...
Connection passwords are stored in Windows Credential Manager for the local
Windows user, outside project SQLite. A project can therefore be shared
...
For referential-integrity rules, two sources on the same database connection
are compared by a SQL `NOT EXISTS` query. Cross-connection and CSV checks use
the application-side comparison path instead.
