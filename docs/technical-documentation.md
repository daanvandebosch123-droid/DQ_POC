...
Connection passwords are kept separately in Windows Credential Manager under the current Windows user. The key includes the application username and connection name, allowing a project to share a connection definition without saving its password in project SQLite. Legacy plaintext JSON entries in the local application-data directory are migrated when first read and removed after a successful Credential Manager write.

Connections can optionally define a daily rule-execution window in `Europe/Brussels` time. Automated schedules cannot run rules using that connection outside the configured window. A manual run outside the window requires explicit confirmation. This applies to both connections in a referential rule, and windows spanning midnight are supported.

This is appropriate for a local/internal proof of concept. A production deployment can use a managed secret vault for centralised credential rotation and should use database accounts with minimum required permissions.
...
When both sides of a referential-integrity rule use the same database connection,
the service evaluates the key comparison inside that database with SQL `NOT
EXISTS`. Only the aggregate counts and a limited failed-row preview return to
the application. Checks spanning connections, or using CSV files, retain the
...
summary queries from the current unsaved form values without opening a database
connection or executing anything. The preview follows the selected source's
...
The optional Ollama service uses a locally running Ollama endpoint (normally `http://localhost:11434`) and the configured model. **Explain with AI** produces a concise anomaly explanation. **Get AI recommendations** prioritizes suggested rule types and operational next steps based on profile statistics, finding metadata, GDPR review categories, and the existing deterministic rule ideas. It sends metadata only: source rows, sampled values, and finding messages are excluded. Recommendations are advisory and do not create, edit, run, or schedule rules. Rule execution does not depend on Ollama.
...
Run one test module or one named test while working on a focused change:

    .\.venv\Scripts\python.exe -m pytest tests\test_execution.py -q
    .\.venv\Scripts\python.exe -m pytest tests\test_execution.py -k referential -q
...
The test suite is intentionally service-focused. It should run without a
browser and without access to a real database.

| Test module | Main behaviour protected |
| --- | --- |
| `test_execution.py` | CSV and database rule execution, thresholds, failed-row evidence, freshness and referential integrity |
| `test_dialects.py` | SQL generated for DuckDB, Oracle, SQL Server, DB2 and Sybase |
| `test_rule_validation.py` | Rule configuration requirements, normalization and invalid settings |
| `test_connection_sql.py` | ODBC/Oracle connection-string construction and driver-specific settings |
| `test_ai.py` | Ollama prompt construction and metadata-only privacy boundary |
| `test_profiling.py` | Profile statistics, rule suggestions, Dutch identifier inference and GDPR findings |
| `test_scheduling.py` | Brussels-time cadence and next-run calculation |
| `test_rule_groups.py` | Nested-group traversal and move behaviour |
| `test_workspace.py` | Workspace, project and membership persistence |
| `test_dashboard_trends.py` | Dashboard history aggregation and empty-data behaviour |
| `test_web_selection.py` | Selection-state helpers used by Rules and Results |

### Writing a unit test

Use a descriptive `test_<expected_behaviour>` name and arrange the test around
one observable outcome. Prefer small CSV fixtures in `tests/fixtures` for
file-backed behaviour. For Oracle or ODBC behaviour, mock the connector or
cursor and assert the generated SQL, connection settings, returned summary, or
error handling; do not add production credentials or require an available
database server.

Tests that create output must use the existing temporary runtime locations and
clean them in `tearDown`/fixtures. Do not commit `tests/.runtime-results`,
`.pytest_cache`, generated CSVs, local SQLite files, or driver output.

Before handing off a change, run the test module nearest to the changed service,
then the full suite when practical, together with Ruff and `git diff --check`.
