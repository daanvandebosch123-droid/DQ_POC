...
from dqtool.services.scheduling import describe_execution_window, is_within_execution_window
...
class ExecutionService:
        ...
        for rule in rules:
            ...
            try:
                if executed_by == "scheduler":
                    self._ensure_connection_execution_windows(rule, datasets, connections)
    ...
    def _ensure_connection_execution_windows(
        self, rule: Rule, datasets: dict[int, Dataset], connections: dict[int, Connection]
    ) -> None:
        unavailable = self.connections_outside_execution_window([rule], datasets, connections)
        if unavailable:
            connection, start, end = unavailable[0]
            window = describe_execution_window(start, end)
            raise RuntimeError(f"Connection '{connection.name}' only allows scheduled execution during {window}.")

    def connections_outside_execution_window(
        self, rules: list[Rule], datasets: dict[int, Dataset], connections: dict[int, Connection]
    ) -> list[tuple[Connection, str, str]]:
        """Return connections that are currently outside their optional execution window."""
        unavailable: dict[int, tuple[Connection, str, str]] = {}
        for rule in rules:
            for connection_id in self._rule_connection_ids(rule, datasets):
                connection = connections.get(connection_id)
                if connection is None:
                    continue
                start = str(connection.config.get("execution_window_start") or "").strip()
                end = str(connection.config.get("execution_window_end") or "").strip()
                if (start or end) and not is_within_execution_window(start, end):
                    unavailable[connection_id] = (connection, start, end)
        return [unavailable[connection_id] for connection_id in sorted(unavailable)]

    def _rule_connection_ids(self, rule: Rule, datasets: dict[int, Dataset]) -> set[int]:
        connection_ids: set[int] = set()
        if self._uses_embedded_source(rule):
            for key in ("source_connection_id", "target_connection_id"):
                if rule.config.get(key) is not None:
                    connection_ids.add(int(rule.config[key]))
            return connection_ids
        dataset = datasets.get(rule.dataset_id or 0)
        if dataset and dataset.connection_id is not None:
            connection_ids.add(dataset.connection_id)
        if rule.rule_type == RuleType.REFERENTIAL_INTEGRITY:
            target_dataset = datasets.get(int(rule.config.get("target_dataset_id") or 0))
            if target_dataset and target_dataset.connection_id is not None:
                connection_ids.add(target_dataset.connection_id)
        return connection_ids

    def preview_rule_sql(self, rule: Rule, connections: dict[int, Connection]) -> list[tuple[str, str]]:
        """Return the SQL that would run for a rule without opening a database connection."""
        ...
        source_connection = connections.get(source_connection_id)
        if source_connection is None:
            raise ValueError("Select a valid source connection before previewing SQL.")
        self._validate_connection_kind(source_connection, rule.config.get("source_kind"), "source")
        ...
        if rule.rule_type == RuleType.DATA_FRESHNESS:
            ...
            return [("Freshness query", f"SELECT COUNT(*), MAX({column}) FROM {relation_name}")]

        if rule.rule_type == RuleType.REFERENTIAL_INTEGRITY:
            target_connection_id = int(rule.config["target_connection_id"])
            target_connection = connections.get(target_connection_id)
            if target_connection is None:
                raise ValueError("Select a valid target connection before previewing SQL.")
            self._validate_connection_kind(target_connection, rule.config.get("target_kind"), "target")
            if source_connection_id != target_connection_id or source_connection.connection_type.value == "csv":
                raise ValueError(
                    "This referential-integrity rule compares sources outside one database query, so it has no generated SQL preview."
                )
            target_sql = self.connector_service.rule_source_sql(self._extract_target_source_config(rule.config))
            failed_sql, summary_sql = self._same_connection_referential_sql(rule, source_sql, target_sql)
            return [("Failed rows query", failed_sql), ("Summary query", summary_sql)]

        dialect = "duckdb" if source_connection.connection_type.value == "csv" else self.connector_service.database_dialect(source_connection)
        relation_name = "dataset_view" if source_connection.connection_type.value == "csv" else f"({source_sql}) q"
        failed_sql, summary_sql = self._build_rule_sql(rule, relation_name, dialect=dialect)
        return [("Failed rows query", failed_sql), ("Summary query", summary_sql)]
    ...
    def _execute_rule(
        ...
        if dataset.dataset_type in LOCAL_DATASET_TYPES:
            if rule.rule_type == RuleType.DATA_FRESHNESS:
                return self._run_freshness_rule(rule, self._dataset_relation_builder(dataset, connections))
            return self._run_duckdb_rule(rule, self._dataset_relation_builder(dataset, connections))
        if rule.rule_type == RuleType.DATA_FRESHNESS:
            return self._run_database_freshness_rule(rule, connections[dataset.connection_id or 0], self._dataset_sql(dataset))
    ...
    def _execute_rule_source(
        ...
        if rule.rule_type == RuleType.DATA_FRESHNESS:
            if connection.connection_type.value == "csv":
                return self._run_freshness_rule(rule, self._rule_source_relation_builder(source_config, connections))
            return self._run_database_freshness_rule(rule, connection, self.connector_service.rule_source_sql(source_config))
        if connection.connection_type.value == "csv":
    ...
    def _run_freshness_rule(self, rule: Rule, build_relation: RelationBuilder) -> tuple[dict[str, Any], list[dict[str, Any]]]:
        con = duckdb.connect()
        try:
            relation = build_relation(con)
            con.sql(f"CREATE OR REPLACE VIEW dataset_view AS {relation.sql_query()}")
            column = self._quote_identifier(rule.config["column"])
            row_count, latest_value = con.execute(f"SELECT COUNT(*), MAX({column}) FROM dataset_view").fetchone()
        finally:
            con.close()
        return self._freshness_result(rule, int(row_count or 0), latest_value)

    def _run_database_freshness_rule(
        self, rule: Rule, connection: Connection, source_sql: str
    ) -> tuple[dict[str, Any], list[dict[str, Any]]]:
        column = self._quote_identifier(rule.config["column"])
        db_conn = self.connector_service.connect_database(connection)
        try:
            with db_conn.cursor() as cursor:
                cursor.execute(f"SELECT COUNT(*), MAX({column}) FROM ({source_sql}) q")
                row_count, latest_value = cursor.fetchone()
        finally:
            db_conn.close()
        return self._freshness_result(rule, int(row_count or 0), latest_value)

    def _freshness_result(self, rule: Rule, row_count: int, latest_value: Any) -> tuple[dict[str, Any], list[dict[str, Any]]]:
        max_age_days = int(rule.config["max_age_days"])
        if max_age_days < 0:
            raise ValueError("Maximum age (days) cannot be negative.")
        latest = self._parse_freshness_value(latest_value)
        if latest is None:
            message = "The source is empty or the selected field has no readable latest date."
            summary = self._summary(rule, row_count, 1)
            summary.update({"latest_value": None, "freshness_age_days": None, "max_age_days": max_age_days, "freshness_message": message})
            return summary, [{"latest_value": "", "max_age_days": max_age_days, "finding": message}]
        age = datetime.now(UTC) - latest
        age_days = max(0, age.total_seconds() / 86400)
        stale = age > timedelta(days=max_age_days)
        summary = self._summary(rule, row_count, int(stale))
        summary.update(
            {
                "latest_value": latest.isoformat(),
                "freshness_age_days": round(age_days, 2),
                "max_age_days": max_age_days,
                "freshness_message": "Source freshness is within the allowed age." if not stale else "Source freshness is outside the allowed age.",
            }
        )
        failed_rows = []
        if stale:
            failed_rows.append(
                {
                    ...
                    "freshness_age_days": round(age_days, 2),
                    "max_age_days": max_age_days,
                    "finding": "Latest source value is too old.",
                }
            )
        return summary, failed_rows

    def _parse_freshness_value(self, value: Any) -> datetime | None:
        if value is None:
            return None
    ...
    def _execute_referential_integrity(
        ...
        connection = self._shared_database_connection(
            source_dataset.connection_id, target_dataset.connection_id, connections
        )
        if connection is not None:
            return self._run_same_connection_referential_integrity(
                rule, connection, self._dataset_sql(source_dataset), self._dataset_sql(target_dataset)
            )
    ...
    def _execute_referential_integrity_rule_sources(
        ...
        if errors:
            raise ValueError(f"Invalid referential integrity rule configuration. {'; '.join(errors)}")
        connection = self._shared_database_connection(
            source_config.get("source_connection_id"), target_source_config.get("source_connection_id"), connections
        )
        if connection is not None:
            return self._run_same_connection_referential_integrity(
                rule,
                connection,
                self.connector_service.rule_source_sql(source_config),
                self.connector_service.rule_source_sql(target_source_config),
            )
    ...
    def _shared_database_connection(
        self,
        source_connection_id: int | None,
        target_connection_id: int | None,
        connections: dict[int, Connection],
    ) -> Connection | None:
        """Return the shared database connection, if both sources can be joined in SQL."""
        if source_connection_id is None or target_connection_id is None:
            return None
        if int(source_connection_id) != int(target_connection_id):
            return None
        connection = connections.get(int(source_connection_id))
        if connection is None or connection.connection_type.value == "csv":
            return None
        return connection

    def _run_same_connection_referential_integrity(
        self,
        rule: Rule,
        ...
        """Evaluate a same-database foreign-key check without transferring keys to Python."""
        failed_sql, summary_sql = self._same_connection_referential_sql(rule, source_sql, target_sql)
        dialect = self.connector_service.database_dialect(connection)
        db_conn = self.connector_service.connect_database(connection)
        try:
    ...
    def _same_connection_referential_sql(self, rule: Rule, source_sql: str, target_sql: str) -> tuple[str, str]:
        source_key = self._quote_identifier(rule.config["source_key"])
        target_key = self._quote_identifier(rule.config["target_key"])
        missing_target_predicate = (
            f"s.{source_key} IS NOT NULL AND NOT EXISTS "
            f"(SELECT 1 FROM ({target_sql}) t WHERE t.{target_key} = s.{source_key})"
        )
        failed_sql = f"SELECT s.* FROM ({source_sql}) s WHERE {missing_target_predicate}"
        summary_sql = (
            f"SELECT COUNT(*), COALESCE(SUM(CASE WHEN {missing_target_predicate} THEN 1 ELSE 0 END), 0) "
            f"FROM ({source_sql}) s"
        )
