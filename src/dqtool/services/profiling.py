    ...
    def _profile_duckdb(
        ...
        try:
            ...
            if columns:
                ...
                count_values = con.execute(
                    f"SELECT {', '.join(f'COUNT({column}), COUNT(DISTINCT {column})' for column in quoted)} FROM profile_view"
                ).fetchone()
                values = iter(count_values)
                for name in columns:
        ...
        for name, stats in columns.items():
            ...
            if date_share >= DOMINANT_SHARE:
                ...
                min_date, max_date = con.execute(
                    f"SELECT MIN(try_cast({quoted} AS DATE)), MAX(try_cast({quoted} AS DATE)) FROM profile_view"
                ).fetchone()
                stats["min"] = self._json_safe(min_date)
                stats["max"] = self._json_safe(max_date)
            ...
            if blank_count:
                findings.append(
                    _finding("medium", name, f"{blank_count} value(s) are blank or contain spaces only.")
                )
            ...
            for start in range(0, len(described), PROFILE_AGGREGATE_BATCH_SIZE):
                ...
                for item in batch:
                    ...
                    if item[0] in numeric_names:
                        aggregates.extend((f"MIN({quoted})", f"MAX({quoted})", f"AVG({quoted})", f"{stddev_function}({quoted})"))
...
def profile_rule_suggestions(profile: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for column, stats in profile.get("columns", {}).items():
        ...
        if row_count and null_rate == 0 and float(stats.get("blank_rate") or 0) == 0:
