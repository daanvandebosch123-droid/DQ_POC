...
RULE_TEMPLATES: dict[RuleType, dict[str, str]] = {
    ...
    RuleType.DATA_FRESHNESS: {
        "name": "Data Freshness",
        "description": "Fails when the newest value in a date or timestamp field is older than the allowed age, or when the source is empty.",
        "setup": "Choose a real date/timestamp field and enter the maximum allowed age in days. For text dates, use Date Validity first and prefer a custom SQL rule when the format is non-standard.",
    },
...
RULE_REQUIRED_CONFIG: dict[RuleType, tuple[str, ...]] = {
    ...
    RuleType.DATA_FRESHNESS: ("column", "max_age_days"),
...
def validate_rule_config(rule_type: RuleType, config: dict, *, require_source: bool = False) -> list[str]:
    ...
    if rule_type == RuleType.DATA_FRESHNESS and "max_age_days" not in missing:
        try:
            if int(normalized["max_age_days"]) < 0:
...
def ancestor_group_ids(group_id: int, groups_by_id: dict[int, RuleGroup]) -> set[int]:
    ...
    for candidate_id in groups_by_id:
