...
class DQToolWebApp:
    def __init__(self, workspace: WorkspaceContext | None = None, user: User | None = None) -> None:
        ...
        self._anomaly_history_key: str = ""
        self._anomaly_history_source_config: dict[str, Any] = {}
        ...
        self._failed_rows_preview_headers: list[str] = []
        self._failed_rows_preview_data: list[dict[str, str]] = []
        self._failed_rows_preview_context = ""
        ...
        self.result_ai_explanation: ui.markdown
    ...
    def _build_anomalies_tab(self) -> None:
        with ui.column().classes("w-full gap-4"):
            ...
            with ui.card().classes("dq-soft-card dq-section-card w-full p-6"):
                ...
                self.anomaly_history_table = self._build_table(
                ...
                with ui.card().classes("dq-soft-card w-full lg:w-[calc(50%-8px)] p-6"):
                    ...
                    self.profile_table = ui.table(
                        columns=[
                            ...
                            {"name": "distinct_rate", "label": "Distinct %", "field": "distinct_rate", "align": "right"},
            ...
            with ui.card().classes("dq-soft-card w-full p-6"):
                ...
                self.profile_suggestions_table = self._build_table(["Rule", "Field", "Why", "Priority"], pagination=8)
    ...
    def _render_anomaly_report(
        ...
        *,
        ...
        self.profile_table.rows = [
            {
                ...
                "distinct_rate": (
                    f"{(float(stats.get('distinct_count') or 0) / int(stats.get('non_null_count') or 0)):.1%}"
        ...
        if previous is None:
            if is_history_view:
                message = (
                    f"This was the baseline snapshot for **{source_label}** "
                    f"({int(profile.get('row_count') or 0):,} rows, {len(profile.get('columns', {}))} columns), "
                    f"taken {self._format_timestamp(profile.get('profiled_at'))}."
                )
            else:
                message = (
    ...
    def _load_anomaly_history(self, _event: Any = None) -> None:
        if not self.project:
            return
        connection = self._anomaly_connection()
        target = str(self.anomaly_target_select.value or "").strip()
        if connection is None or not target:
            ...
            self.anomaly_history_table.update()
        ...
        rows = []
        for index, snapshot in enumerate(self._anomaly_history):
            previous_snapshot = self._anomaly_history[index - 1] if index > 0 else None
            anomalies = detect_anomalies(previous_snapshot, snapshot)
            rows.append(
                {
                    "id": str(index),
                    "checked": self._format_timestamp(snapshot.get("profiled_at")),
                    "row_count": snapshot.get("row_count", "-"),
                    "findings": len(anomalies),
                    "high_severity": sum(1 for finding in anomalies if finding["severity"] == "high"),
                    "gdpr_flags": len(snapshot.get("gdpr_findings") or []),
                }
            )
        rows.reverse()
        self.anomaly_history_table.rows = rows
        self.anomaly_history_table.update()
        if self._anomaly_history:
            self.anomaly_history_status.text = (
                f"{len(self._anomaly_history)} past check(s) for {target}. Click a row to view it."
            )
        else:
            self.anomaly_history_status.text = f"No past checks yet for {target}. Run a check to create the first snapshot."
        self.anomaly_history_status.update()

    def _view_anomaly_history_row(self, event: Any) -> None:
        row = self._row_from_click_event(event)
        if row is None or not self._anomaly_history:
            return
        try:
            index = int(row["id"])
        except (KeyError, TypeError, ValueError):
            return
        if not (0 <= index < len(self._anomaly_history)):
            return
        ...
        self._highlight_table_row(self.anomaly_history_table, row["id"])
        self._set_last_action(f"Viewed anomaly check history for {target} ({self._format_timestamp(profile.get('profiled_at'))})")
    ...
    def _refresh_profile_suggestions(self) -> None:
        ...
        self.profile_suggestions_table.rows = [
            {
                "id": str(index),
                "rule": suggestion["name"],
                "field": suggestion["column"],
                "why": self._suggestion_ai_notes.get(index, {}).get("why") or suggestion["reason"],
                "priority": self._suggestion_ai_notes.get(index, {}).get("priority", "-"),
            }
    ...
    async def prioritize_profile_suggestions(self) -> None:
        report = self._last_anomaly_report
        if not report or not self._profile_suggestions:
        ...
        try:
            ...
            ui.notify(f"Could not prioritize rule ideas: {exc}", type="negative")
            return
        self._suggestion_ai_notes = notes
        self._refresh_profile_suggestions()
        self._set_last_action("Prioritized rule ideas with AI")
        ui.notify("Rule ideas prioritized." if notes else "The model returned no usable priorities.", type="positive" if notes else "warning")
    ...
    async def recommend_ai_actions(self) -> None:
        report = self._last_anomaly_report
        if not report:
            ui.notify("Run an anomaly check first.", type="warning")
            return
        ...
        try:
            text = await nicegui_run.io_bound(
                self.ollama_service.recommend_actions,
                report["source_label"],
                report["profile"],
                report["anomalies"],
                self._profile_suggestions,
            ...
            ui.notify(str(exc), type="negative")
            return
        self.ai_recommendations.content = text or "The model returned no recommendations."
        self.ai_recommendations.update()
        self._set_last_action("Generated AI rule recommendations and next steps")
    ...
    def show_connection_dialog(self, existing: Connection | None = None) -> None:
        ...
        with ui.dialog() as dialog, ui.card().classes("dq-scroll-dialog w-[760px] max-w-full"):
            ...
            execution_window_enabled = ui.checkbox(
                "Restrict rule execution to a daily time window",
                value=bool(existing_config.get("execution_window_start") or existing_config.get("execution_window_end")),
            )
            execution_window_start = ui.input(
                "Allowed from", value=str(existing_config.get("execution_window_start") or "")
            ).props("type=time").classes("w-full")
            execution_window_end = ui.input(
                "Allowed until", value=str(existing_config.get("execution_window_end") or "")
            ).props("type=time").classes("w-full")
            execution_window_help = ui.label(
                "Uses Europe/Brussels time. Overnight windows (for example 22:00–06:00) are supported."
            ...
            def sync_visibility() -> None:
                ...
                execution_window_start.visible = bool(execution_window_enabled.value)
                execution_window_end.visible = bool(execution_window_enabled.value)
                execution_window_help.visible = bool(execution_window_enabled.value)
                for element in (
                    ...
                    execution_window_start, execution_window_end, execution_window_help,
                ):
                    element.update()
            ...
            execution_window_enabled.on_value_change(lambda _event: sync_visibility())
            sync_visibility()
            ...
            def build_connection(require_name: bool) -> Connection:
                ...
                if execution_window_enabled.value:
                    window_error = validate_execution_window(execution_window_start.value, execution_window_end.value)
                    if window_error:
                        raise ValueError(window_error)
                    config["execution_window_start"] = str(execution_window_start.value)
                    config["execution_window_end"] = str(execution_window_end.value)
                else:
                    config.pop("execution_window_start", None)
                    config.pop("execution_window_end", None)
                ...
                try:
                    ...
                    if selected_type != ConnectionType.CSV:
                        if password.value:
                            ...
                            if existing and (existing.name, str(existing.config.get("username", ""))) != (
                                connection.name,
                                config.get("username", ""),
                            ):
                                delete_connection_secret(existing.name, str(existing.config.get("username", "")))
                        elif existing:
                            ...
                            if (existing.name, old_username) != (connection.name, new_username):
                                ...
                                if old_secret:
                                    ...
                                    delete_connection_secret(existing.name, old_username)
                    dialog.close()
                    self.refresh_all()
    ...
    def show_ai_rule_draft_dialog(self) -> None:
        ...
        if not connections:
            ...
            return
        connections_by_id = {item.id: item for item in connections.values() if item.id is not None}
        connection_options = {
            key: f"{item.name} · {item.connection_type.value.upper()}"
            for key, item in connections.items()
        }
        with ui.dialog() as dialog, ui.card().classes("w-[640px] max-w-full gap-3"):
            with ui.row().classes("items-center gap-3"):
                with ui.element("div").classes("grid w-10 h-10 rounded-xl bg-stone-200 place-items-center"):
            ...
            draft_connection = ui.select(
                connection_options,
                value=next(iter(connections)) if len(connections) == 1 else None,
                label="Source connection *",
                clearable=True,
            ).props("outlined options-dense").classes("w-full")
            draft_target = ui.select(
                [], label="File / table *", with_input=True, clearable=True
            ).props("outlined options-dense").classes("w-full")
            draft_target_status = ui.label("Select a connection to load its files or tables.").classes(
                "text-xs text-[#837d74]"
            )
            draft_description = ui.textarea(
            ...
            async def load_targets() -> None:
                await self._load_connection_targets(draft_connection, draft_target)
                connection = connections.get(str(draft_connection.value))
                if connection is None:
                    draft_target_status.text = "Select a connection to load its files or tables."
                else:
                    noun = "CSV file" if connection.connection_type == ConnectionType.CSV else "table or view"
                    count = len(draft_target.options or [])
                    draft_target_status.text = f"Found {count} {noun}{'' if count == 1 else 's'}."
                draft_target_status.update()

            draft_connection.on_value_change(lambda _event: load_targets())

            async def draft() -> None:
                description_text = (draft_description.value or "").strip()
                if not description_text:
                    ui.notify("Describe the check you want first.", type="warning")
                    return
                connection = connections.get(str(draft_connection.value))
                target = str(draft_target.value or "").strip()
                if connection is None or not target:
                    ui.notify("Select a source connection and a file or table first.", type="warning")
                    return
                available = await nicegui_run.io_bound(self.ollama_service.is_available)
                if not available:
                    ui.notify(
                ...
                draft_status.text = "Loading source fields..."
                draft_status.update()
                try:
                    columns = await nicegui_run.io_bound(
                        self.connector_service.list_rule_source_columns, source_config, connections_by_id
                    )
                except Exception as exc:
                    draft_status.text = f"Could not load fields from that source: {exc}"
                    draft_status.update()
                    ui.notify(str(exc), type="negative")
                    return
                if not columns:
                    draft_status.text = "That source has no fields to draft a rule from."
                    draft_status.update()
                    return
                sample_values: dict[str, list[str]] | None = None
                if include_sample_values.value:
                    draft_status.text = "Loading sample values..."
                    draft_status.update()
                    try:
                        preview_columns, preview_rows, _stats = await nicegui_run.io_bound(
                            self.connector_service.preview_rule_source, source_config, connections_by_id
                        )
                    except Exception as exc:
                        draft_status.text = f"Could not load sample values: {exc}"
                        draft_status.update()
                        ui.notify(str(exc), type="negative")
                        return
                    sample_values = {
                ...
                try:
                    draft_result = await nicegui_run.io_bound(
                        self.ollama_service.draft_rule, description_text, columns, sample_values
                    )
                except Exception as exc:
                    draft_status.text = f"Could not draft a rule: {exc}"
                    draft_status.update()
                    ui.notify(str(exc), type="negative")
                    return
                ...
                self.show_rule_dialog(
                    suggestion={
                        "name": str(draft_result.get("name") or "").strip()
                        or f"AI draft: {RULE_TEMPLATES[selected_type]['name']}",
                        "rule_type": selected_type.value,
                        "config": {**source_config, **drafted_config},
                        "description": f"Drafted by AI from: {description_text}",
                    }
                )

            with ui.row().classes("justify-end gap-2 w-full"):
                ui.button("Cancel", on_click=dialog.close).props("flat")
                ui.button("Draft", icon="auto_awesome", on_click=draft).props("color=primary")
        dialog.open()
        if len(connections) == 1:
            ui.timer(0.1, load_targets, once=True)
    ...
    def show_rule_dialog(self, rule: Rule | None = None, suggestion: dict[str, Any] | None = None) -> None:
        ...
        with ui.dialog() as dialog, ui.card().classes("dq-rule-dialog w-[980px] max-w-full"):
            ...
            with ui.column().classes("dq-reference-panel gap-3"):
                ...
                with ui.row().classes("w-full gap-3 flex-wrap") as freshness_fields:
                    max_age_days = ui.number("Maximum age (days)", value=1, min=0, format="%.0f").classes("grow min-w-[180px]")
            ...
            def rule_config_from_form() -> tuple[RuleType, dict[str, Any]]:
                ...
                merged.update(
                    {
                        "source_connection_id": int(source_connection.value),
                        "source_kind": source_kind.value,
                        "source_name": str(source_name.value or "").strip(),
                        "source_sql": str(source_sql.value or "").strip(),
                    }
                )
                    ...
                    merged.update(
                        {
                            ...
                            "target_name": str(target_name.value or "").strip(),
                            "target_sql": str(target_sql.value or "").strip(),
                        }
                    )
                setting_keys = {
                ...
                if selected_type in {
                    RuleType.NOT_NULL, RuleType.VALUE_RANGE, RuleType.REGEX, RuleType.LENGTH,
                    RuleType.ALLOWED_VALUES, RuleType.DATE_VALIDITY, RuleType.DATA_FRESHNESS,
                }:
                ...
                if errors:
                    raise ValueError("\n".join(errors))
                return selected_type, normalized

            def preview_generated_sql() -> None:
                try:
                    selected_type, normalized = rule_config_from_form()
                    preview_rule = Rule(
                        id=rule.id if rule else None,
                        name=(name.value or "").strip() or "SQL preview",
                        rule_type=selected_type,
                        dataset_id=None,
                        owner_username=rule.owner_username if rule else self.current_user,
                        config=normalized,
                    )
                    sql_parts = self.execution_service.preview_rule_sql(
                        preview_rule, {int(key): value for key, value in connections.items()}
                    )
                except Exception as exc:
                    ui.notify(str(exc), type="warning")
                    return
                with ui.dialog() as preview_dialog, ui.card().classes("w-[980px] max-w-full"):
                    ui.label("Generated SQL preview").classes("text-xl font-semibold")
                    ui.label("This is generated from the current unsaved form values; it does not execute the rule.").classes(
                        "dq-panel-copy text-sm"
                    )
                    for label, sql in sql_parts:
                        ui.label(label).classes("text-sm font-medium mt-3")
                        ui.textarea(value=sql).props("readonly outlined autogrow").classes("w-full font-mono")
            ...
            with ui.row().classes("justify-end gap-2 w-full"):
                ui.button("Cancel", on_click=dialog.close).props("flat")
                ui.button("Preview generated SQL", icon="code", on_click=preview_generated_sql).props("outline no-caps")
                ui.button("Save", on_click=save).props("color=primary")
        dialog.open()
    ...
    async def run_selected_schedule_now(self) -> None:
        ...
        await self._confirm_execution_window_override(rules, execute)
    ...
    def _confirm_run_group(
        ...
        with ui.dialog() as dialog, ui.card().classes("w-[560px] max-w-full"):
            ...
            async def confirm() -> None:
                dialog.close()
                await self._confirm_execution_window_override(
                    rules, lambda: self._execute_group_rules(group, rules, missing_rules, missing_groups)
                )
    ...
    async def _confirm_execution_window_override(self, rules: list[Rule], on_confirm: Any) -> None:
        """Ask for explicit confirmation before a manual run overrides a connection window."""
        connections = {connection.id: connection for connection in self._visible_connections() if connection.id is not None}
        try:
            unavailable = self.execution_service.connections_outside_execution_window(rules, {}, connections)
        except Exception as exc:
            ui.notify(str(exc), type="negative")
            return
        if not unavailable:
            await on_confirm()
            return
        windows = ", ".join(f"{connection.name} ({start}–{end})" for connection, start, end in unavailable)
        with ui.dialog() as dialog, ui.card().classes("w-[560px] max-w-full"):
            ui.label("Outside connection execution window").classes("text-xl font-semibold")
            ui.label(
                f"This manual run is outside the allowed window for: {windows}. Do you want to run it anyway?"
            ).classes("text-sm")
            ...
            with ui.row().classes("justify-end gap-2 w-full"):
                ui.button("Cancel", on_click=dialog.close).props("flat")
                ui.button("Run anyway", icon="warning", on_click=confirm).props("color=warning unelevated")
        dialog.open()
    ...
    async def run_selected_rule(self) -> None:
        ...
        await self._confirm_execution_window_override([rule], execute)
    ...
    def _confirm_run_checked_rules(self, rules: list[Rule], missing: int) -> None:
        ...
        with ui.dialog() as dialog, ui.card().classes("w-[560px] max-w-full"):
            ...
            async def confirm() -> None:
                dialog.close()
                await self._confirm_execution_window_override(rules, lambda: self._execute_checked_rules(rules, missing))
    ...
    async def explain_selected_result(self) -> None:
        run = self._selected_result()
        if run is None or not self.project:
            ui.notify("Select a result first.", type="warning")
            return
        if run.status == "passed":
            ui.notify("This run passed; there is nothing to explain.", type="info")
            return
        rule = next((item for item in self.project.storage.list_rules() if item.id == run.rule_id), None)
        if rule is None:
            ui.notify("The rule for this run no longer exists.", type="warning")
            return
        available = await nicegui_run.io_bound(self.ollama_service.is_available)
        if not available:
            ui.notify(
        ...
        try:
            text = await nicegui_run.io_bound(
                self.ollama_service.explain_rule_failure,
                rule.name,
            ...
            ui.notify(str(exc), type="negative")
            return
        self.result_ai_explanation.content = text or "The model returned an empty response."
        self.result_ai_explanation.update()
        self._set_last_action(f"Generated AI explanation for run {run.id}")
    ...
    def preview_selected_failed_rows(self) -> None:
        ...
        self._failed_rows_preview_headers = headers
        self._failed_rows_preview_data = [
            {header: (row.get(header) or "") for header in headers}
            for row in data_rows[:200]
        ]
        self._failed_rows_preview_context = f"{rule_name} on {source_name}"
        self._failed_rows_preview_total = int(run.summary_json.get("failed_count") or len(self._failed_rows_preview_data))
        self.failed_rows_column_select.options = {"__all__": "All columns", **{header: header for header in headers}}
        self.failed_rows_column_select.value = "__all__"
        self.failed_rows_column_select.update()
        self.failed_rows_search.value = ""
        self.failed_rows_search.update()
        self._refresh_failed_rows_preview()
        self._set_last_action(f"Previewed failed rows for run {run.id}")

    def _clear_failed_rows_preview(self) -> None:
        self._failed_rows_preview_headers = []
        self._failed_rows_preview_data = []
        self._failed_rows_preview_context = ""
        self._failed_rows_preview_total = 0
        self.failed_rows_search.value = ""
        self.failed_rows_search.update()
        self.failed_rows_column_select.options = {"__all__": "All columns"}
        self.failed_rows_column_select.value = "__all__"
        self.failed_rows_column_select.update()
    ...
    def _clear_failed_rows_filter(self) -> None:
        self.failed_rows_search.value = ""
        self.failed_rows_column_select.value = "__all__"
        self.failed_rows_search.update()
        self.failed_rows_column_select.update()
    ...
    def _refresh_failed_rows_preview(self) -> None:
        query = str(self.failed_rows_search.value or "").strip().lower()
        selected_column = str(self.failed_rows_column_select.value or "__all__")
        filtered_rows = []
        for row in self._failed_rows_preview_data:
            values = row.values() if selected_column == "__all__" else [row.get(selected_column, "")]
            if not query or any(query in str(value).lower() for value in values):
                filtered_rows.append(row)
        self.failed_rows_table.columns = [
            {"name": header, "label": header, "field": header, "align": "left", "sortable": True}
            for header in self._failed_rows_preview_headers
        ]
        self.failed_rows_table.rows = [
            {"id": index, **row}
        ...
        if self._failed_rows_preview_context:
            ...
            self.failed_rows_label.text = (
                f"{self._failed_rows_preview_context} | Showing {len(filtered_rows):,} matching row(s) "
                f"from {previewed:,} previewed of {self._failed_rows_preview_total:,} failed row(s)"
            )
            self.failed_rows_label.update()
