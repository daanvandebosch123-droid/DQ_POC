...
class DialectSqlTests(unittest.TestCase):
    ...
    def test_sql_preview_uses_the_execution_queries_without_connecting(self) -> None:
        ...
        rule = _rule(
            RuleType.NOT_NULL,
            {
                ...
                "source_name": "SPAR.KLANT_INFO",
                "source_sql": "",
                "column": "CLIENT_ID",
            },
        )

        queries = self.service.preview_rule_sql(rule, {1: connection})

        self.assertEqual(["Failed rows query", "Summary query"], [label for label, _sql in queries])
        self.assertIn('FROM (SELECT * FROM SPAR.KLANT_INFO) q WHERE "CLIENT_ID" IS NULL', queries[0][1])
        self.assertIn("checked_count", queries[1][1])

    def test_sql_preview_shows_same_connection_referential_query(self) -> None:
        connection = Connection(
            id=1,
            name="iq",
            connection_type=ConnectionType.SYBASE,
        ...
        self.assertIn("NOT EXISTS", queries[0][1])
        self.assertIn("FROM (SELECT * FROM SPAR.CUSTOMERS) t", queries[0][1])
