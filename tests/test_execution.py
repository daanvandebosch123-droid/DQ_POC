...
class ExecutionServiceTests(unittest.TestCase):
    ...
    def test_scheduler_rule_errors_when_its_connection_is_outside_execution_window(self) -> None:
        self.csv_connection.config.update({"execution_window_start": "09:00", "execution_window_end": "17:00"})
        rule = Rule(
            id=1,
            name="unique ids",
            rule_type=RuleType.UNIQUE,
            dataset_id=None,
            owner_username="tester",
            config={"source_connection_id": 11, "source_kind": "csv_file", "source_name": "customers.csv", "columns": ["id"]},
        )

        with patch("dqtool.services.execution.is_within_execution_window", return_value=False):
            run = self.service.run_rules([rule], {}, {11: self.csv_connection}, self.results_dir, "scheduler")[0]

        self.assertEqual("error", run.status)
        self.assertIn("only allows scheduled execution during 09:00–17:00 Brussels time", run.summary_json["error"])

    def test_manual_rule_can_override_connection_execution_window(self) -> None:
        self.csv_connection.config.update({"execution_window_start": "09:00", "execution_window_end": "17:00"})
        rule = Rule(
            id=1,
            name="unique ids",
    ...
    def test_data_freshness_fails_when_latest_value_is_too_old(self) -> None:
        rule = Rule(
            id=1,
        ...
        summary, failed_rows = self.service._freshness_result(
            rule, 10, datetime.now(UTC) - timedelta(days=2)
        )
    ...
    def test_referential_integrity_on_one_database_connection_uses_sql(self) -> None:
        connection = Connection(
            id=21,
            name="quality-db",
        ...
        cursor = MagicMock()
        cursor.description = [("customer_id",)]
        cursor.fetchall.return_value = [(999,)]
        cursor.fetchone.return_value = (3, 1)
        db_connection = MagicMock()
        db_connection.cursor.return_value.__enter__.return_value = cursor

        with (
            patch.object(self.service.connector_service, "connect_database", return_value=db_connection),
            patch.object(self.service.connector_service, "database_dialect", return_value="sybase"),
            patch.object(self.service.connector_service, "limited_sql", side_effect=lambda sql, _limit, _dialect: sql),
        ):
            run = self.service.run_rules([rule], {}, {21: connection}, self.results_dir, "tester")[0]

        self.assertEqual("failed", run.status)
        self.assertEqual(3, run.summary_json["checked_count"])
        self.assertEqual(1, run.summary_json["failed_count"])
        self.assertIn("NOT EXISTS", cursor.execute.call_args_list[0].args[0])
        self.assertIn('FROM (SELECT * FROM orders) s', cursor.execute.call_args_list[1].args[0])
        self.assertNotIn("fetchmany", str(cursor.mock_calls))
