...
class RuleValidationTests(unittest.TestCase):
    ...
    def test_data_freshness_requires_a_field_and_maximum_age(self) -> None:
        errors = validate_rule_config(RuleType.DATA_FRESHNESS, {"column": "loaded_at"})

        self.assertIn("Missing required setting: max_age_days", errors)

    def test_data_freshness_rejects_negative_maximum_age(self) -> None:
        errors = validate_rule_config(RuleType.DATA_FRESHNESS, {"column": "loaded_at", "max_age_days": -1})
