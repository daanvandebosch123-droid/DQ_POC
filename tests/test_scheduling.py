...
class ComputeNextRunTests(unittest.TestCase):
    def test_execution_window_accepts_daytime_and_overnight_ranges(self) -> None:
        noon = datetime(2026, 8, 7, 12, 0, tzinfo=BRUSSELS_TIMEZONE)
        night = datetime(2026, 8, 7, 23, 0, tzinfo=BRUSSELS_TIMEZONE)

        self.assertTrue(is_within_execution_window("09:00", "17:00", noon))
        self.assertFalse(is_within_execution_window("09:00", "17:00", night))
        self.assertTrue(is_within_execution_window("22:00", "06:00", night))
        self.assertEqual("Execution window start and end times must be different.", validate_execution_window("09:00", "09:00"))
